...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            1, 
            "", 
            ["kubectl command failed", "exit code 1"]
        ),
    ],
    ids=[
        "success-json",
...
async def test_kubectl_tool_parametrized(
    command, timeout, json_output, result_exit_code, result_stdout, 
    expected_strings, test_assertions, test_factory
):
    """
        ...
        with patch("mcp_server_troubleshoot.server.get_kubectl_executor") as mock_get_executor:
            ...
            if result_exit_code != 0:
                ...
                mock_executor.execute = AsyncMock(
                    side_effect=KubectlError(f"kubectl command failed: {command}", result_exit_code, "")
                )
            else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "grep_files",
            {"pattern": "pattern", "path": "dir1", "recursive": True, 
             "glob_pattern": "*.txt", "case_sensitive": False, "max_results": 100},
            GrepResult(
                pattern="pattern",
                path="dir1",
                glob_pattern="*.txt",
        ...
        (
            "grep_files",
            {"pattern": "common", "path": ".", "recursive": True, 
             "glob_pattern": "*.txt", "case_sensitive": False, "max_results": 100},
            GrepResult(
                pattern="common",
                path=".",
    ...
    with patch("mcp_server_troubleshoot.server.get_file_explorer") as mock_get_explorer:
        ...
        if file_operation == "list_files":
            ...
            mock_explorer.grep_files.assert_awaited_once_with(
                args["pattern"], args["path"], args["recursive"], 
                args["glob_pattern"], args["case_sensitive"], args["max_results"]
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            False,
            False,
            ["No support bundles found", "download or transfer a bundle"]
        ),
