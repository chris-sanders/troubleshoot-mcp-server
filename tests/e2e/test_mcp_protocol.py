...
class MCPClient:
    ...
    async def send_request(self, method, params=None, timeout=5.0):
        ...
        if self.process.returncode is not None:
            return {
                "jsonrpc": "2.0",
                "error": {"code": -32603, "message": f"Process exited with code {self.process.returncode}"},
...
async def mcp_server():
    ...
    env["MCP_LOG_LEVEL"] = "ERROR"  # Minimize output to avoid hanging
    ...
    try:
        ...
        # Test the connection with a simple ping (list_tools)
        try:
            response = await client.list_tools(timeout=5.0)
            if not response or "error" in response:
                # Force terminate the process if we get an error
                process.terminate()
                try:
...
@pytest.mark.asyncio
@pytest.mark.timeout(15)  # Add timeout to prevent hanging
