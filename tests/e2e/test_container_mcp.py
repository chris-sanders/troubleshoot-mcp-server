...
class DockerMCPClient:
    ...
    def __init__(self, process, container_id=None):
        ...
        self.request_id = 0
        # Add debugging flag - set to True to see detailed logging
        self.debug = os.environ.get("MCP_CLIENT_DEBUG", "false").lower() in ("true", "1", "yes")
        
        # Set a shorter timeout for tests
        self.timeout = float(os.environ.get("MCP_CLIENT_TIMEOUT", "5.0"))
        print(f"DockerMCPClient initialized with timeout={self.timeout}s, debug={self.debug}")
    ...
    def send_request(self, method, params=None):
        ...
        if self.process.poll() is not None:
            error_msg = f"Container process exited with code {self.process.returncode}"
            print(error_msg)
            return {
                "jsonrpc": "2.0",
        ...
        max_attempts = 15
        attempt_interval = 0.2  # Wait 0.2 seconds between attempts

        for attempt in range(max_attempts):
            # Check if we've exceeded the timeout
            elapsed = time.time() - start_time
            if elapsed > self.timeout:
                error_msg = f"Timeout waiting for response after {elapsed:.1f}s"
                print(error_msg)
                return {
                    "jsonrpc": "2.0",
                    "error": {"code": -32000, "message": error_msg},
            ...
            # Check if process is still running
            if self.process.poll() is not None:
                error_msg = f"Container process exited with code {self.process.returncode} while waiting for response"
                print(error_msg)
                return {
                    "jsonrpc": "2.0",
                    "error": {"code": -32000, "message": error_msg},
                    ...
                    if response_line:
                        try:
                            ...
                            try:
                                ...
                                # Log the error and try to continue reading
                                print(f"Error decoding response (attempt {attempt+1}): {e}")
                                print(f"Response text: {response_str}")
                                ...
                                if attempt >= max_attempts - 1:
                                    return {
                                        ...
                                        "error": {
                                            "code": -32700,
                                            "message": f"Response was not valid JSON: {response_str}",
                                        },
                                        "id": str(self.request_id),
                                    }
                        except UnicodeDecodeError as e:
                            print(f"Error decoding response bytes (attempt {attempt+1}): {e}")
                            # Try to decode with replacement for invalid bytes
                            try:
                                response_str = response_line.decode("utf-8", errors="replace").strip()
                                print(f"Response with replacement: {response_str}")
                            except Exception:
                                pass
                            ...
                            if attempt >= max_attempts - 1:
                                return {
                                    ...
                                    "error": {
                                        "code": -32700,
                                        "message": "Response contained invalid UTF-8 bytes",
                                    },
                                    "id": str(self.request_id),
                                }
            ...
            try:
                ...
                if ready:
                    ...
                    if stderr_data:
                        ...
                        if self.debug or "error" in stderr_str.lower():
                            print(f"Container stderr: {stderr_str}")
            except Exception as e:
                if self.debug:
            ...
            if self.debug and attempt > 0:
                print(f"Waiting for response (attempt {attempt+1}/{max_attempts}, elapsed: {elapsed:.1f}s)")
            time.sleep(attempt_interval)

        # If we get here, we failed to get a response after all attempts
        error_msg = f"Failed to get response after {max_attempts} attempts ({time.time() - start_time:.1f}s)"
        print(error_msg)
        return {
            "jsonrpc": "2.0",
            "error": {"code": -32000, "message": error_msg},
