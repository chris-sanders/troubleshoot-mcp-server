...
async def bundle_manager(test_support_bundle, clean_asyncio):
    """
    ...
    Returns:
        A tuple of (BundleManager, bundle_path) for use in tests
    """
...
def test_sbctl_help_behavior(test_support_bundle):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "--support-bundle-location" in serve_help_output, "Serve command should document bundle location option"
...
@pytest.mark.asyncio
async def test_file_explorer_behavior(bundle_manager):
    """
    ...
    if root_list.entries:
        ...
        if dir_entries:
            ...
            assert recursive_list.total_files + recursive_list.total_dirs > 0, "Recursive listing should find files/dirs"
            ...
            if all_files:
                ...
                assert hasattr(file_content, "content"), "File content should have content attribute"
...
async def test_bundle_manager_performance(bundle_manager):
    ...
    assert duration < 45.0, f"Initialization should complete in reasonable time (took {duration:.2f}s)"
