...
class InitializeBundleArgs(BaseModel):
    ...
    """
    source: str = Field(description="The source of the bundle (URL or local path)")
    force: bool = Field(False, description="Whether to force re-initialization if a bundle is already active")
...
class BundleManager:
    ...
    async def _download_bundle(self, url: str) -> Path:
        ...
        filename = os.path.basename(parsed_url.path) or f"bundle_{self._generate_bundle_id(url)}.tar.gz"
    ...
    async def _wait_for_initialization(self, kubeconfig_path: Path, timeout: float = 30.0) -> None:
        ...
        raise BundleInitializationError(f"Timeout waiting for bundle initialization after {timeout} seconds")
