...
async def integration_components(mock_bundle_path):
    ...
    with mock.patch.dict(os.environ, env_vars):
        ...
        with mock.patch("mcp_server_troubleshoot.server.mcp") as mock_mcp:
            # Mock tool registry
            mock_mcp.tool_registry = {
                "initialize_bundle": mock.MagicMock(
                    name="initialize_bundle", description="Initialize a support bundle"
                ),
                "kubectl": mock.MagicMock(name="kubectl", description="Execute kubectl commands"),
                "list_files": mock.MagicMock(
                    name="list_files", description="List directory contents"
                ),
                "read_file": mock.MagicMock(name="read_file", description="Read file contents"),
                "grep_files": mock.MagicMock(
                    name="grep_files", description="Search for patterns in files"
                ),
            }

            # Create mock server for compatibility with existing tests
            mcp_server = mock.MagicMock()
            mcp_server.list_tools = mock.MagicMock(
                return_value=[
                    {"name": "initialize_bundle", "description": "Initialize a support bundle"},
        ...
        mock_result.stdout = json.dumps(
            {
                "items": [
                    {
                        "metadata": {"name": "sample-pod", "namespace": "default"},
...
async def test_mcp_server_call_tool(integration_components):
    ...
    result1 = await mcp_server.call_tool("files__list_directory", {"path": "/"})
    ...
    result2 = await mcp_server.call_tool(
        "files__read_file", {"path": "/kubernetes/pods/sample-pod.yaml"}
    )

    assert result2 == "Sample file content"

    # Test calling kubectl__execute tool
    result3 = await mcp_server.call_tool("kubectl__execute", {"command": "get pods"})
    ...
    mcp_server.call_tool.assert_any_call(
        "files__read_file", {"path": "/kubernetes/pods/sample-pod.yaml"}
    )
...
async def test_error_handling(integration_components):
    ...
    with mock.patch.object(
        file_explorer, "read_file", side_effect=FileNotFoundError("File not found")
    ):
...
def test_container_mcp_communication():
    ...
    try:
        ...
        assert (
            image_check.stdout.strip()
        ), "Docker image mcp-server-troubleshoot:latest does not exist"
...
def test_clean_stdio_in_mcp_mode():
    ...
    try:
        ...
        try:
            ...
            request = {"jsonrpc": "2.0", "id": "1", "method": "get_tool_definitions"}
